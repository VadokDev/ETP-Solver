...
\KwData{
\begin{itemize}
	\item[] actualExm $\leftarrow$ Examen con el cual se verificará si el dominio filtrado debe reiniciarse o no
	\item[] $D_{i}$ $\leftarrow$ Lista de horarios asignables (dominio) del examen $i$, $D_{i}$[$j$] se marca con el examen del cual se realizó el FC para filtar el horario $j$
	\item[] E $\leftarrow$ Cantidad de exámenes
\end{itemize}
}
\SetKwProg{maxSlotsAdjusment}{Procedure}{}{End}
\maxSlotsAdjusment{\resetDomains{actualExam}}{
	...
	\While{exam $\leq$ E}{
		\For{horario in $D_{exam}$}{
			\If{si $D_{i}$[horario] está marcado por actualExam}{
				$D_{i}$[horario] = 0\;
			}
		}
...
\KwResult{Establecer arco-consistencia entre los dominios de los exámenes por asignar y las asignaciones ya realizadas, marcando los valores incompatibles con el examen que inició el FC}
\KwData{
\begin{itemize}
	\item[] $E \leftarrow$ Cantidad de Exámenes
	\item[] $L \leftarrow$ Conjunto de Horarios
	\item[] $cMatrix_{E \times E} \leftarrow$ Matriz de conflictos entre exámenes, $cMatrix_{ij}$ es 1 si el examen $i$ tiene conflictos con el examen $j$
	\item[] $D_{i}$ $\leftarrow$ Lista de horarios asignables (dominio) del examen $i$, $D_{i}$[$j$] se marca con el examen del cual se realizó el FC para filtar el horario $j$
	\item[] $sol_{i}$ $\leftarrow$ Lista que representa la solución actual, donde $sol_{i}$ equivale al horario asignado al examen $i$
\end{itemize}
}
\SetKwProg{fc}{Procedure}{}{XDDD}
\fc{\doForwardChecking{actualExam}}{
exam $\leftarrow$ actualExam + 1\;
\While{exam $\leq$ E}{
	\If{$D_{exam}$[$sol_{actualExam}$] no está marcado}{
		\If{$cMatrix_{actualExam, exam}$}{
			ola
		}
	}	
	exam $\leftarrow$ exam + 1\;
